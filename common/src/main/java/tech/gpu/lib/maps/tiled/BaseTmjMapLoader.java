        ...
        if (encoding == null || encoding.isEmpty() || encoding.equals("csv")) {
            ...
            try {
                ...
                if (compression == null || compression.isEmpty())
                    ...
                    throw new GpuRuntimeException("Unrecognised compression (" + compression + ") for TMJ Layer Data");
                ...
                for (int y = 0; y < height; y++) {
                    for (int x = 0; x < width; x++) {
                        ...
                        if (read != temp.length)
                            throw new GpuRuntimeException("Error Reading TMJ Layer Data: Premature end of tile data");
                        ids[y * width + x] = unsignedByteToInt(temp[0]) | unsignedByteToInt(temp[1]) << 8
                                | unsignedByteToInt(temp[2]) << 16 | unsignedByteToInt(temp[3]) << 24;
                    }
                }
            } catch (IOException e) {
                throw new GpuRuntimeException("Error Reading TMJ Layer Data - IOException: " + e.getMessage());
            } finally {
            ...
            // or another editor
            throw new GpuRuntimeException("Unrecognised encoding (" + encoding + ") for TMJ Layer Data");
        }

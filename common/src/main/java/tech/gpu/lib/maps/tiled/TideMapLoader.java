...
import tech.gpu.lib.maps.ImageResolver.AssetManagerImageResolver;
import tech.gpu.lib.maps.ImageResolver.DirectImageResolver;
...
import tech.gpu.lib.utils.XmlReader;
    ...
    public TiledMap load(String fileName) {
        try {
            ...
            for (FileHandle textureFile : loadTileSheets(root, tideFile)) {
                textures.put(textureFile.path(), new Texture(ApplicationEnvironment.app, textureFile.file()));
            }
            DirectImageResolver imageResolver = new DirectImageResolver(textures);
            TiledMap map = loadMap(root, tideFile, imageResolver);
            map.setOwnedResources(textures.values().toArray());
            return map;
        } catch (IOException | PixelFormatNotSupportedException e) {
            throw new GpuRuntimeException("Couldn't load tilemap '" + fileName + "'", e);
        }

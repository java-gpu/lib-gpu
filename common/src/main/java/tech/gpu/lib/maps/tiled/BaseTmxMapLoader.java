...
import tech.gpu.lib.assets.loaders.TextureLoader;
...
import tech.gpu.lib.maps.*;
import tech.gpu.lib.maps.objects.*;
import tech.gpu.lib.maps.tiled.TiledMapTileLayer.Cell;
import tech.gpu.lib.maps.tiled.objects.TiledMapTileMapObject;
import tech.gpu.lib.maps.tiled.tiles.AnimatedTiledMapTile;
import tech.gpu.lib.maps.tiled.tiles.StaticTiledMapTile;
import tech.gpu.lib.math.Polygon;
import tech.gpu.lib.math.Polyline;
import tech.gpu.lib.utils.*;
import tech.gpu.lib.utils.XmlReader.Element;
...
public abstract class BaseTmxMapLoader<P extends BaseTiledMapLoader.Parameters> extends BaseTiledMapLoader<P> {
    ...
    protected void loadClassProperties(String className, MapProperties classProperties, XmlReader.Element classElement) {
        ...
        if (projectClassMembers == null) {
            throw new GpuRuntimeException("There is no class with name '" + className + "' in given Tiled project file.");
        }
    ...
    static public int[] getTileIds(Element element, int width, int height) {
        ...
        if (encoding == null) { // no 'encoding' attribute means that the encoding is XML
            throw new GpuRuntimeException("Unsupported encoding (XML) for TMX Layer Data");
        }
            ...
            if (true) if (encoding.equals("base64")) {
                ...
                try {
                    ...
                    if (compression == null)
                        ...
                        throw new GpuRuntimeException("Unrecognised compression (" + compression + ") for TMX Layer Data");
                    ...
                    for (int y = 0; y < height; y++) {
                        for (int x = 0; x < width; x++) {
                            ...
                            if (read != temp.length)
                                throw new GpuRuntimeException("Error Reading TMX Layer Data: Premature end of tile data");
                            ids[y * width + x] = unsignedByteToInt(temp[0]) | unsignedByteToInt(temp[1]) << 8
                                    | unsignedByteToInt(temp[2]) << 16 | unsignedByteToInt(temp[3]) << 24;
                        }
                    }
                } catch (IOException e) {
                    throw new GpuRuntimeException("Error Reading TMX Layer Data - IOException: " + e.getMessage());
                } finally {
                ...
                // or another editor
                throw new GpuRuntimeException("Unrecognised encoding (" + encoding + ") for TMX Layer Data");
            }
        }
        return ids;
    }
        ...
        if (element.getName().equals("tileset")) {
            ...
            if (source != null) {
                ...
                try {
                    ...
                    throw new GpuRuntimeException("Error parsing external tileset.");
                }
            } else {
                Element imageElement = element.getChildByName("image");
                if (imageElement != null) {

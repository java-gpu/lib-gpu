...
//            throw new GpuRuntimeException("InstanceBufferObject requires a device running with GLES 3.0 compatibilty");
//
...
//        setUsage(isStatic ? GL20.GL_STATIC_DRAW : GL20.GL_DYNAMIC_DRAW);
    ...
    protected void setBuffer(Buffer data, boolean ownsBuffer, VertexAttributes value) {
        if (isBound) throw new GpuRuntimeException("Cannot change attributes while VBO is bound");
        if (this.ownsBuffer && byteBuffer != null) BufferUtils.disposeUnsafeByteBuffer(byteBuffer);
        attributes = value;
        if (data instanceof ByteBuffer)
            byteBuffer = (ByteBuffer) data;
        else
            throw new GpuRuntimeException("Only ByteBuffer is currently supported");
    ...
    private void bufferChanged() {
        if (isBound) {
//            ApplicationEnvironment.gl20.glBufferData(GL20.GL_ARRAY_BUFFER, byteBuffer.limit(), null, usage);
//            ApplicationEnvironment.gl20.glBufferData(GL20.GL_ARRAY_BUFFER, byteBuffer.limit(), byteBuffer, usage);
            // TODO
            isDirty = false;
        }
    }
    ...
    protected void setUsage(int value) {
        if (isBound) throw new GpuRuntimeException("Cannot change usage while VBO is bound");
        usage = value;
    }
...
//        final GL20 gl = ApplicationEnvironment.gl20;
//
...
//                ApplicationEnvironment.gl30.glVertexAttribDivisor(location + unitOffset, 1);
//            }
//
//        } else {
//            for (int i = 0; i < numAttributes; i++) {

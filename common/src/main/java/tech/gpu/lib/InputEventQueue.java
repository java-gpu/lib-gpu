...
public class InputEventQueue {
    ...
    private final List<Integer> processingQueue = new LinkedList<>();
    @Getter
    ...
    public void drain(InputProcessor processor) {
        ...
        Integer[] q = processingQueue.toArray(new Integer[]{});
        for (int i = 0, n = q.length; i < n; ) {
            int type = q[i++];
            ...
            switch (type) {
                ...
                case KEY_UP:
                    ...
                    break;
                case KEY_TYPED:
                    processor.keyTyped((char) (int) q[i++]);
                    break;
                case TOUCH_DOWN:
                ...
                case MOUSE_MOVED:
                    ...
                    break;
                case SCROLLED:
                    processor.scrolled(Float.intBitsToFloat(q[i++]), Float.intBitsToFloat(q[i++]));
                    break;
                default:
    ...
    private synchronized int next(int nextType, int i) {
        Integer[] q = queue.toArray(new Integer[]{});
        for (int n = q.length; i < n; ) {
            int type = q[i];
    ...
    public synchronized boolean scrolled(float amountX, float amountY, long time) {
        queue.add(SCROLLED);
        queueTime(time);
        queue.add(Float.floatToIntBits(amountX));
        queue.add(Float.floatToIntBits(amountY));
        return false;
    }

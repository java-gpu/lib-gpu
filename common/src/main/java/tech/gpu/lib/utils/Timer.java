...
import tech.gpu.lib.LifecycleListener;
...
public class Timer {
    ...
    static private TimerThread thread() {
        synchronized (threadLock) {
            if (thread == null || thread.files != ApplicationEnvironment.files) {
    ...
    static abstract public class Task implements Runnable {
        ...
        public Task() {
            app = ApplicationEnvironment.app; // Store which app to postRunnable (eg for multiple LwjglAWTCanvas).
            if (app == null) throw new IllegalStateException("ApplicationEnvironment.app not available.");
        }
    ...
    static class TimerThread implements Runnable, LifecycleListener {
        ...
        public TimerThread() {
            files = ApplicationEnvironment.files;
            app = ApplicationEnvironment.app;
        ...
        public void run() {
            while (true) {
                synchronized (threadLock) {
                    if (thread != this || files != ApplicationEnvironment.files) break;
                    ...
                    if (pauseTimeMillis == 0) {
                        ...
                        for (int i = 0, n = instances.size; i < n; i++) {
                            try {
                                ...
                                throw new GpuRuntimeException("Task failed: " + instances.get(i).getClass().getName(), ex);
                            }
                        }
                    }

...
/**
 * An unordered map where the keys are objects and the values are unboxed floats. keys are not allowed. No allocation is
...
public class ObjectFloatMap<K> implements Iterable<ObjectFloatMap.Entry<K>> {
    ...
    public int hashCode() {
        ...
        for (int i = 0, n = keyTable.length; i < n; i++) {
            K key = keyTable[i];
            if (key != null) h += key.hashCode() + Float.floatToIntBits(valueTable[i]);
        }
        return h;
    }

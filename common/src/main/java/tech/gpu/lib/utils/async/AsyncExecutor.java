...
package tech.gpu.lib.utils.async;
...
public class AsyncExecutor implements Disposable {
    ...
    public <T> AsyncResult<T> submit(final AsyncTask<T> task) {
        if (executor.isShutdown()) {
            throw new GpuRuntimeException("Cannot run tasks on an executor that has been shutdown (disposed)");
        }
    ...
    public void dispose() {
        ...
        try {
            ...
            throw new GpuRuntimeException("Couldn't shutdown loading thread", e);
        }
    }
}

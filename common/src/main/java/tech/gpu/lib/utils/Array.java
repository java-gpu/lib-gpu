...
import tech.gpu.lib.utils.reflect.ArrayReflection;
...
public class Array<T> implements Iterable<T> {
     ...
     * array must have a size greater than 0, or a {@link GpuRuntimeException} will be thrown.
     *
    ...
    public T selectRanked(Comparator<T> comparator, int kthLowest) {
        if (kthLowest < 1) {
            throw new GpuRuntimeException("nth_lowest must be greater than 0, 1 = first, 2 = second...");
        }
    ...
    /**
     * Returns a random item from the array, or if the array is empty.
     */
    public T random() {
...
// System.out.println(iterable.lastAcquire);
                throw new GpuRuntimeException("#iterator() cannot be used nested.");
            }
...
// System.out.println(iterable.lastAcquire);
        ...
        public void remove() {
            if (!allowRemove) throw new GpuRuntimeException("Remove not allowed.");
            index--;

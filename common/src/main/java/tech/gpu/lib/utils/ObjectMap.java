...
import static tech.gpu.lib.utils.ObjectSet.tableSize;

/**
 * An unordered map where the keys and values are objects. keys are not allowed. No allocation is done except when growing
 * the table size.
 * <p>
...
public class ObjectMap<K, V> implements Iterable<ObjectMap.Entry<K, V>> {
    ...
    /**
     * Returns the value for the specified key, or if the key is not in the map.
     */
    ...
    /**
     * Returns the value for the removed key, or if the key is not in the map.
     */
    ...
    /**
     * Returns the key for the specified value, or if it is not in the map. Note this traverses the entire map and compares
    ...
    static public class Entries<K, V> extends MapIterator<K, V, Entry<K, V>> {
        ...
        public Entry<K, V> next() {
            ...
            if (!valid) throw new GpuRuntimeException("#iterator() cannot be used nested.");
            K[] keyTable = map.keyTable;

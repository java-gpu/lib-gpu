...
import tech.gpu.lib.scenes.scene2d.Group;
 ...
 * @author Nathan Sweet
 */
public class Tree<N extends Tree.Node, V> extends WidgetGroup implements Styleable<Tree.TreeStyle> {
     ...
     *
     * @param parent for the root nodes.
    ...
    protected Drawable getExpandIcon(N node, float iconX) {
        if (node == overNode //
            ...
            float mouseX = screenToLocalCoordinates(tmp.set(ApplicationEnvironment.input.getX(), 0)).x + getX();

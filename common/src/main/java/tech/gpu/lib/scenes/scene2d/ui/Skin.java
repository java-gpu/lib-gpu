...
import tech.gpu.lib.ex.SerializationException;
import tech.gpu.lib.files.FileHandle;
import tech.gpu.lib.graphics.Color;
import tech.gpu.lib.graphics.Texture;
import tech.gpu.lib.graphics.g2d.*;
import tech.gpu.lib.graphics.g2d.BitmapFont.BitmapFontData;
import tech.gpu.lib.graphics.g2d.TextureAtlas.AtlasRegion;
import tech.gpu.lib.graphics.g2d.TextureAtlas.AtlasSprite;
import tech.gpu.lib.scenes.scene2d.Actor;
import tech.gpu.lib.scenes.scene2d.utils.*;
import tech.gpu.lib.utils.*;
import tech.gpu.lib.utils.Json.ReadOnlySerializer;
import tech.gpu.lib.utils.reflect.ClassReflection;
import tech.gpu.lib.utils.reflect.Method;
 ...
 * <p>
 * See the <a href="https://libApplicationEnvironment.com/wiki/graphics/2d/scene2d/skin">documentation</a> for more.
 *
 * @author Nathan Sweet
 */
public class Skin implements Disposable {
     ...
     *
     * @throws GpuRuntimeException if the resource was not found.
     */
    ...
    public <T> T get(String name, Class<T> type) {
        ...
        ObjectMap<String, Object> typeResources = resources.get(type);
        if (typeResources == null)
            throw new GpuRuntimeException("No " + type.getName() + " registered with name: " + name);
        Object resource = typeResources.get(name);
        if (resource == null) throw new GpuRuntimeException("No " + type.getName() + " registered with name: " + name);
        return (T) resource;
    }
    ...
    /**
     * Returns the name to resource mapping for the specified type, or if no resources of that type exist.
     */
    ...
    public TextureRegion getRegion(String name) {
        ...
        if (texture == null) throw new GpuRuntimeException("No TextureRegion or Texture registered with name: " + name);
    ...
    /**
     * @return an array with the {@link TextureRegion} that have an index != -1, or if none are found.
     */
    ...
    public NinePatch getPatch(String name) {
        ...
        try {
            ...
            return patch;
        } catch (GpuRuntimeException ex) {
            throw new GpuRuntimeException("No NinePatch, TextureRegion, or Texture registered with name: " + name);
        }
    }
    ...
    public Drawable getDrawable(String name) {
        ...
        } catch (GpuRuntimeException ignored) {
        }
    ...
    /**
     * Returns the name of the specified style object, or if it is not in the skin. This compares potentially every style
    ...
    public Drawable newDrawable(Drawable drawable) {
        ...
        throw new GpuRuntimeException("Unable to copy, unknown drawable type: " + drawable.getClass());
    }
    ...
    protected Json getJsonLoader(final FileHandle skinFile) {
                ...
                if (jsonMap.has(parentFieldName)) {
                    ...
                    while (true) {
                        try {
                            ...
                            break;
                        } catch (GpuRuntimeException ex) { // Parent resource doesn't exist.
        ...
        json.setSerializer(BitmapFont.class, new ReadOnlySerializer<BitmapFont>() {
            public BitmapFont read(Json json, JsonValue jsonData, Class type) {
                ...
                if (!fontFile.exists()) fontFile = ApplicationEnvironment.files.internal(path);

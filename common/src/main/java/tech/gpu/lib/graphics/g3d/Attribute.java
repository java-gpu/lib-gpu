...
public abstract class Attribute implements Comparable<Attribute> {
    ...
    /**
     * @return The alias of the specified attribute type, or if not available.
     */
     ...
     * @throws GpuRuntimeException if maximum attribute count reached
     */
    protected final static long register(final String alias) {
        ...
        if (types.size >= MAX_ATTRIBUTE_COUNT) {
            throw new GpuRuntimeException("Cannot register " + alias + ", maximum registered attribute count reached.");
        }
        types.add(alias);

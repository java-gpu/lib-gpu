...
import tech.gpu.lib.graphics.g2d.BitmapFont.Glyph;
...
public class BitmapFontCache {
    ...
    public BitmapFontCache(BitmapFont font, boolean integer) {
        ...
        int pageCount = font.regions.toArray().length;
        if (pageCount == 0)
    ...
    public void tint(Color tint) {
        float newTint = NumberUtils.toFloatBits(tint);
            ...
            for (int ii = 0, nn = layout.runs.size; ii < nn; ii++) {
                ...
                for (int iii = 0, nnn = run.glyphs.size; iii < nnn; iii++) {
                    if (glyphIndex++ == nextColorGlyphIndex) {
                        tempColor = ColorUtil.abgr8888ToColor(colors.get(++colorsIndex));
                        lastColorFloatBits = NumberUtils.toFloatBits(ColorUtil.mul(tempColor, tint));
    ...
    public void setColors(Color tint) {
        setColors(NumberUtils.toFloatBits(tint));
    }
    ...
    public void setColors(Color tint, int start, int end) {
        setColors(NumberUtils.toFloatBits(tint), start, end);
    }
    ...
    public void draw(Batch spriteBatch, float alphaModulation) {
        ...
        float newAlpha = color.getAlpha() * alphaModulation;
        setColors(new Color(color.getRed(), color.getGreen(), color.getBlue(), newAlpha));
        draw(spriteBatch);
        setColors(new Color(color.getRed(), color.getGreen(), color.getBlue(), newAlpha));
    }
    ...
    private void addToCache(GlyphLayout layout, float x, float y) {
        ...
        currentTint = NumberUtils.toFloatBits(new Color(1, 1, 1, 1)); // Cached glyphs have changed, reset the current tint.
    }

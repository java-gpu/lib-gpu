...
package tech.gpu.lib.graphics.g3d;
...
import tech.gpu.lib.graphics.g3d.utils.MeshBuilder;
import tech.gpu.lib.graphics.g3d.utils.RenderableSorter;
import tech.gpu.lib.utils.Array;
import tech.gpu.lib.utils.Disposable;
import tech.gpu.lib.utils.FlushablePool;
...
public class ModelCache implements Disposable, RenderableProvider {
    ...
    public interface MeshPool extends Disposable {
         ...
         * @return the obtained Mesh, or when no mesh could be obtained.
         */
    ...
    public void begin(Camera camera) {
        if (building) throw new GpuRuntimeException("Call end() after calling begin()");
        building = true;
     ...
     */
    public void end() {
        if (!building) throw new GpuRuntimeException("Call begin() prior to calling end()");
        building = false;
     ...
     * cache is used. If the {@link Renderable#bones} member is not then skinning is assumed and the renderable will be added
    ...
    public void add(Renderable renderable) {
        if (!building)
            throw new GpuRuntimeException("Can only add items to the ModelCache in between .begin() and .end()");
    ...
    public void getRenderables(Array<Renderable> renderables, Pool<Renderable> pool) {
        if (building) throw new GpuRuntimeException("Cannot render a ModelCache in between .begin() and .end()");
    ...
    @Override
    public void dispose() {
        if (building) throw new GpuRuntimeException("Cannot dispose a ModelCache in between .begin() and .end()");
        meshPool.dispose();
    }
}

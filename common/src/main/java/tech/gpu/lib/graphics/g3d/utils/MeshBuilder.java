...
import tech.gpu.lib.graphics.g3d.model.MeshPart;
import tech.gpu.lib.graphics.g3d.utils.shapebuilders.*;
import tech.gpu.lib.math.*;
...
public class MeshBuilder implements MeshPartBuilder {
    ...
    /**
     * @param usage bitwise mask of the {@link tech.gpu.lib.graphics.VertexAttributes.Usage}, only Position, Color, Normal and
     ...
     *
     * @param attributes bitwise mask of the {@link tech.gpu.lib.graphics.VertexAttributes.Usage}, only Position, Color, Normal
    ...
    public void begin(final VertexAttributes attributes, int primitiveType) {
        ...
        if (a == null) throw new GpuRuntimeException("Cannot build mesh without position attribute");
    ...
    public Mesh end(Mesh mesh) {
        ...
        if (attributes == null) throw new GpuRuntimeException("Call begin() first");
        if (!attributes.equals(mesh.getVertexAttributes()))
            throw new GpuRuntimeException("Mesh attributes don't match");
        if ((mesh.getMaxVertices() * stride) < vertices.size) throw new GpuRuntimeException(
                "Mesh can't hold enough vertices: " + mesh.getMaxVertices() + " * " + stride + " < " + vertices.size);
        if (mesh.getMaxIndices() < indices.size)
            throw new GpuRuntimeException("Mesh can't hold enough indices: " + mesh.getMaxIndices() + " < " + indices.size);
    ...
    public void getVertices(float[] out, int destOffset) {
        if (attributes == null) throw new GpuRuntimeException("Must be called in between #begin and #end");
        if ((destOffset < 0) || (destOffset > out.length - vertices.size))
            throw new GpuRuntimeException("Array too small or offset out of range");
...
//        else
        // TODO
        throw new GpuRuntimeException("Incorrect primtive type");
    }
...
//        else
    ...
    public short vertex(Vector3 pos, Vector3 nor, Color col, Vector2 uv) {
        if (vindex > MAX_INDEX) throw new GpuRuntimeException("Too many vertices used");
...
//        if (primitiveType != GL20.GL_LINES) throw new GpuRuntimeException("Incorrect primitive type");
//        index(index1, index2);
        // TODO
    }
...
//        } else
        // TODO
        throw new GpuRuntimeException("Incorrect primitive type");
    }
...
//        } else
    ...
    public void addMesh(MeshPart meshpart) {
        if (meshpart.primitiveType != primitiveType) throw new GpuRuntimeException("Primitive type doesn't match");
    ...
    public void addMesh(Mesh mesh, int indexOffset, int numIndices) {
        if (!attributes.equals(mesh.getVertexAttributes()))
            throw new GpuRuntimeException("Vertex attributes do not match");

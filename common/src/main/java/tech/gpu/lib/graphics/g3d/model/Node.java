...
public class Node {
     ...
     * @return The node with the specified id, or if not found.
     */
    ...
    public <T extends Node> int insertChild(int index, final T child) {
        for (Node p = this; p != null; p = p.getParent()) {
            if (p == child) throw new GpuRuntimeException("Cannot add a parent as a child");
        }
        Node p = child.getParent();
        if (p != null && !p.removeChild(child))
            throw new GpuRuntimeException("Could not remove child from its current parent");

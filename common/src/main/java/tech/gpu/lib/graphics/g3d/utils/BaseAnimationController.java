...
import tech.gpu.lib.graphics.g3d.ModelInstance;
import tech.gpu.lib.graphics.g3d.model.Animation;
import tech.gpu.lib.graphics.g3d.model.Node;
import tech.gpu.lib.graphics.g3d.model.NodeAnimation;
import tech.gpu.lib.graphics.g3d.model.NodeKeyframe;
...
import tech.gpu.lib.utils.Pool.Poolable;
...
public class BaseAnimationController {
    ...
    protected void begin() {
        if (applying) throw new GpuRuntimeException("You must call end() after each call to being()");
        applying = true;
    }
    ...
    protected void apply(final Animation animation, final float time, final float weight) {
        if (!applying) throw new GpuRuntimeException("You must call begin() before adding an animation");
     ...
     */
    protected void end() {
        if (!applying) throw new GpuRuntimeException("You must call begin() first");
    ...
    protected void applyAnimation(final Animation animation, final float time) {
        if (applying) throw new GpuRuntimeException("Call end() first");
    ...
    protected void applyAnimations(final Animation anim1, final float time1, final Animation anim2, final float time2,
        ...
        if (anim2 == null || weight == 0.f)
            ...
            throw new GpuRuntimeException("Call end() first");
        else {
            begin();

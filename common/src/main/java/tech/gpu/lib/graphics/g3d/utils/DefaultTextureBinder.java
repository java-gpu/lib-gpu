...
public final class DefaultTextureBinder implements TextureBinder {
    ...
    public DefaultTextureBinder(final int method, final int offset, int count) {
        ...
        if (offset < 0 || count < 0 || (offset + count) > max) throw new GpuRuntimeException("Illegal arguments");
        this.method = method;
        this.offset = offset;
        this.count = count;
    ...
    private static int getMaxTextureUnits() {
        ...
        // TODO
//        ApplicationEnvironment.gl.glGetIntegerv(GL20.GL_MAX_TEXTURE_IMAGE_UNITS, buffer);
        return buffer.get(0);
    }
    ...
    public void end() {
        /*
         * No need to unbind and textures are set to in begin() for(int i = 0; i < count; i++) { if (textures[i] != null) {
         * ApplicationEnvironment.gl.glActiveTexture(GL20.GL_TEXTURE0 + offset + i); ApplicationEnvironment.gl.glBindTexture(GL20.GL_TEXTURE_2D, 0); textures[i] = null; }
         * }
         */
//        ApplicationEnvironment.gl.glActiveTexture(GL20.GL_TEXTURE0);
        // TODO
    }
...
//                texture.bind(result);
//            else
//                ApplicationEnvironment.gl.glActiveTexture(GL20.GL_TEXTURE0 + result);
        } else
            bindCount++;
...
//        texture.bind(offset + currentTexture);
    ...
    private final int bindTextureLRU(final Texture texture) {
        int i;
        for (i = 0; i < count; i++) {

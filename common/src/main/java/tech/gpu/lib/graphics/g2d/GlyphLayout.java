...
import tech.gpu.lib.graphics.Colors;
...
public class GlyphLayout implements Poolable {
     ...
     * @param truncate    If not and the width of the glyphs exceed targetWidth, the glyphs are truncated and the glyphs for the
    ...
    public void setText(BitmapFont font, CharSequence str, int start, int end, Color color, float targetWidth, int halign,
        ...
        int currentColor = NumberUtils.toIntBits(color), nextColor = currentColor;
     ...
     *
     * @return May be if second run is all whitespace.
     */
    ...
    private int parseColorMarkup(CharSequence str, int start, int end) {
        ...
        for (int i = start + 1; i < end; i++) {
            char ch = str.charAt(i);
            if (ch != ']') continue;
            tech.gpu.lib.graphics.Color color = Colors.get(str.subSequence(start, i).toString());

...
package tech.gpu.lib.graphics.g3d.shaders;
...
import tech.gpu.lib.graphics.g3d.*;
import tech.gpu.lib.graphics.g3d.attributes.*;
import tech.gpu.lib.graphics.g3d.environment.AmbientCubemap;
import tech.gpu.lib.graphics.g3d.environment.DirectionalLight;
import tech.gpu.lib.graphics.g3d.environment.PointLight;
import tech.gpu.lib.graphics.g3d.environment.SpotLight;
import tech.gpu.lib.graphics.g3d.utils.RenderContext;
...
public class DefaultShader extends BaseShader {
    public static class Config {
        /**
         * The uber vertex shader to use, to use the default vertex shader.
         */
        public String vertexShader = null;
        /**
         * The uber fragment shader to use, to use the default fragment shader.
         */
    ...
    public static String getDefaultVertexShader() {
        if (defaultVertexShader == null)
            defaultVertexShader = ApplicationEnvironment.files.classpath("com/badlogic/gdx/graphics/g3d/shaders/default.vertex.glsl").readString();
    ...
    public static String getDefaultFragmentShader() {
        if (defaultFragmentShader == null)
            defaultFragmentShader = ApplicationEnvironment.files.classpath("com/badlogic/gdx/graphics/g3d/shaders/default.fragment.glsl").readString();
    ...
    public DefaultShader(final Renderable renderable, final Config config, final ShaderProgram shaderProgram) {
        ...
        if (!config.ignoreUnimplemented && (implementedFlags & attributesMask) != attributesMask)
            throw new GpuRuntimeException("Some attributes not implemented yet (" + attributesMask + ")");

        if (renderable.bones != null && renderable.bones.length > config.numBones) {
            throw new GpuRuntimeException("too many bones: " + renderable.bones.length + ", max configured: " + config.numBones);
        }

        int boneWeights = renderable.meshPart.mesh.getVertexAttributes().getBoneWeights();
        if (boneWeights > config.numBoneWeights) {
            throw new GpuRuntimeException("too many bone weights: " + boneWeights + ", max configured: " + config.numBoneWeights);
        }
    ...
    public void begin(final Camera camera, final RenderContext context) {
        ...
        if (has(u_time)) set(u_time, time += ApplicationEnvironment.graphics.getDeltaTime());
...
//                if (location >= 0) {
//                    ApplicationEnvironment.gl.glVertexAttrib2f(location, 0, 0);
//                }
//            }
//        }
    }
    ...
    protected void bindMaterial(final Attributes attributes) {
        ...
        for (final Attribute attr : attributes) {
            ...
            if (BlendingAttribute.is(t)) {
                ...
                throw new GpuRuntimeException("Unknown material attribute: " + attr.toString());
        }

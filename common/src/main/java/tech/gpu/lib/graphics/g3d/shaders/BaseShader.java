...
import tech.gpu.lib.utils.IntIntMap;
...
public abstract class BaseShader implements Shader {
    ...
    public int register(final String alias, final Validator validator, final Setter setter) {
        if (locations != null) throw new GpuRuntimeException("Cannot register an uniform after initialization");
    ...
    public void init(final ShaderProgram program, final Renderable renderable) {
        if (locations != null) throw new GpuRuntimeException("Already initialized");
        if (!program.isCompiled()) throw new GpuRuntimeException(program.getLog());

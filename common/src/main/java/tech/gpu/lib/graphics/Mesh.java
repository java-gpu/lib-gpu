...
public class Mesh implements Disposable {
    ...
    protected Mesh(VertexData vertices, IndexData indices, boolean isVertexArray) {
        ...
        addManagedMesh(ApplicationEnvironment.app, this);
    }
...
//        if (ApplicationEnvironment.gl30 != null) {
...
//        }
         // TODO
        vertices = makeVertexBuffer(true, maxVertices, attributes);
    ...
    public Mesh enableInstancedRendering(boolean isStatic, int maxInstances, VertexAttribute... attributes) {
        if (!isInstanced) {
            ...
            throw new GpuRuntimeException("Trying to enable InstancedRendering on same Mesh instance twice."
    ...
    public Mesh setInstanceData(float[] instanceData, int offset, int count) {
        if (instances != null) {
            ...
            throw new GpuRuntimeException("An InstanceBufferObject must be set before setting instance data!");
        }
        return this;
    }
    ...
    public void render(ShaderProgram shader, int primitiveType, int offset, int count, boolean autoBind) {
        ...
        if (isVertexArray) {
            if (indices.getNumIndices() > 0) {
                ...
                // TODO ApplicationEnvironment.gl20.glDrawElements(primitiveType, count, GL20.GL_UNSIGNED_SHORT, buffer);
                ((Buffer) buffer).position(oldPosition);
            } else {
                // TODO ApplicationEnvironment.gl20.glDrawArrays(primitiveType, offset, count);
            }
        } else {
            int numInstances = 0;
            ...
            if (indices.getNumIndices() > 0) {
                if (count + offset > indices.getNumMaxIndices()) {
                    throw new GpuRuntimeException("Mesh attempting to access memory outside of the index buffer (count: " + count
                            + ", offset: " + offset + ", max: " + indices.getNumMaxIndices() + ")");
                }

//                if (isInstanced && numInstances > 0) {
//                    ApplicationEnvironment.gl30.glDrawElementsInstanced(primitiveType, count, GL20.GL_UNSIGNED_SHORT, offset * 2, numInstances);
//                } else {
//                    ApplicationEnvironment.gl20.glDrawElements(primitiveType, count, GL20.GL_UNSIGNED_SHORT, offset * 2);
//                }
                // TODO
            } else {
//                if (isInstanced && numInstances > 0) {
//                    ApplicationEnvironment.gl30.glDrawArraysInstanced(primitiveType, offset, count, numInstances);
//                } else {
//                    ApplicationEnvironment.gl20.glDrawArrays(primitiveType, offset, count);
//                }
                // TODO
            }
        }
     ...
     */
    public void dispose() {
        if (meshes.get(ApplicationEnvironment.app) != null)
            meshes.get(ApplicationEnvironment.app).removeValue(this, true);
        vertices.dispose();
     ...
     * @return the VertexAttribute or if no attribute with that usage was found.
     */
     ...
     * {@link GpuRuntimeException} is thrown. This method creates a new BoundingBox instance.
     *
    ...
    public BoundingBox extendBoundingBox(final BoundingBox out, int offset, int count, final Matrix4 transform) {
        ...
        if (offset < 0 || count < 1 || offset + count > max)
            throw new GpuRuntimeException("Invalid part specified ( offset=" + offset + ", count=" + count + ", max=" + max + " )");
    ...
    public float calculateRadiusSquared(final float centerX, final float centerY, final float centerZ, int offset, int count,
        ...
        if (offset < 0 || count < 1 || offset + count > numIndices) throw new GpuRuntimeException("Not enough indices");

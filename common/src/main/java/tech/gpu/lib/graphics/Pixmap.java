...
public class Pixmap implements Disposable {
    ...
    public static Pixmap createFromFrameBuffer(int x, int y, int w, int h) {
        // TODO uncomment
//        ApplicationEnvironment.gl.glPixelStorei(GL20.GL_PACK_ALIGNMENT, 1);

        final Pixmap pixmap = new Pixmap(w, h, PixelFormat.RGBA8888);
        ByteBuffer pixels = pixmap.getPixels();
//        ApplicationEnvironment.gl.glReadPixels(x, y, w, h, GL20.GL_RGBA, GL20.GL_UNSIGNED_BYTE, pixels);
    ...
    public Pixmap(int width, int height, PixelFormat format) {
        pixmap = new Gdx2DPixmap(width, height, PixelFormat.toGdx2DPixmapFormat(format));
        setColor(0, 0, 0, 0);
        fill();
    }
    ...
    public Pixmap(byte[] encodedData, int offset, int len) {
        try {
            ...
            throw new GpuRuntimeException("Couldn't load pixmap from image data", e);
        }
    }
    ...
    public Pixmap(ByteBuffer encodedData, int offset, int len) {
        if (!encodedData.isDirect()) throw new GpuRuntimeException("Couldn't load pixmap from non-direct ByteBuffer");
        try {
            pixmap = new Gdx2DPixmap(encodedData, offset, len, 0);
        } catch (IOException e) {
            throw new GpuRuntimeException("Couldn't load pixmap from image data", e);
    ...
    public Pixmap(FileHandle file) {
        try {
            ...
            throw new GpuRuntimeException("Couldn't load file: " + file, e);
        }
    }
...
//        request.setUrl(url);
//        ApplicationEnvironment.net.sendHttpRequest(request, new Net.HttpResponseListener() {
//            @Override
//            public void handleHttpResponse(Net.HttpResponse httpResponse) {
//                final byte[] result = httpResponse.getResult();
//                ApplicationEnvironment.app.postRunnable(new Runnable() {
//                    @Override
//                    public void run() {
//                        try {
...
// public void setStrokeWidth (int width);
    ...
    public void dispose() {
        if (disposed) {
            ApplicationEnvironment.app.error("Pixmap", "Pixmap already disposed!");
            return;
        }
        pixmap.dispose();
        disposed = true;
    ...
    public ByteBuffer getPixels() {
        if (disposed) throw new GpuRuntimeException("Pixmap already disposed");
    ...
    public void setPixels(ByteBuffer pixels) {
        if (!pixels.isDirect()) throw new GpuRuntimeException("Couldn't setPixels from non-direct ByteBuffer");
    ...
    public PixelFormat getFormat() {
        return PixelFormat.fromGdx2DPixmapFormat(pixmap.getFormat());
    }

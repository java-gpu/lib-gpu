...
import tech.gpu.lib.graphics.g3d.utils.*;
...
public class ModelBatch implements Disposable {
    ...
    public void begin(final Camera cam) {
        if (camera != null) throw new GpuRuntimeException("Call end() first.");
        camera = cam;
    ...
    public void setCamera(final Camera cam) {
        if (camera == null) throw new GpuRuntimeException("Call begin() first.");
     ...
     *
     * @return The current camera being used or if called outside {@link #begin(Camera)} and {@link #end()}.
     */

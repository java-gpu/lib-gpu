...
import tech.gpu.lib.utils.NumberUtils;
...
public class NinePatch {
    ...
    public NinePatch(TextureRegion... patches) {
        ...
        if ((patches[TOP_LEFT] != null && patches[TOP_LEFT].getRegionWidth() != leftWidth)
            ...
            throw new GpuRuntimeException("Left side patches must have the same width");
        }
        if ((patches[TOP_RIGHT] != null && patches[TOP_RIGHT].getRegionWidth() != rightWidth)
            ...
            throw new GpuRuntimeException("Right side patches must have the same width");
        }
        if ((patches[BOTTOM_LEFT] != null && patches[BOTTOM_LEFT].getRegionHeight() != bottomHeight)
            ...
            throw new GpuRuntimeException("Bottom side patches must have the same height");
        }
        if ((patches[TOP_LEFT] != null && patches[TOP_LEFT].getRegionHeight() != topHeight)
            ...
            throw new GpuRuntimeException("Top side patches must have the same height");
        }
    }
...
//        tmpDrawColor.set(ColorUtil.mul(color, batch.getColor()));
        final float c = NumberUtils.toFloatBits(tmpDrawColor);

...
package tech.gpu.lib.graphics.g3d.loader;
...
import tech.gpu.lib.graphics.g3d.model.data.*;
...
import tech.gpu.lib.utils.ArrayMap;
import tech.gpu.lib.utils.BaseJsonReader;
import tech.gpu.lib.utils.JsonValue;
...
public class G3dModelLoader extends ModelLoader<ModelLoader.ModelParameters> {
    ...
    public ModelData parseModel(FileHandle handle) {
        ...
        if (model.version[0] != VERSION_HI || model.version[1] != VERSION_LO)
            throw new GpuRuntimeException("Model version not supported");
        ...
        if (meshes != null) {
            ...
            for (JsonValue mesh = meshes.child; mesh != null; mesh = mesh.next) {
                ...
                for (JsonValue meshPart = meshParts.child; meshPart != null; meshPart = meshPart.next) {
                    ...
                    if (partId == null) {
                        throw new GpuRuntimeException("Not id given for mesh part");
                    }
                    for (ModelMeshPart other : parts) {
                        if (other.id.equals(partId)) {
                            throw new GpuRuntimeException("Mesh part with id '" + partId + "' already in defined");
                        }
                    }
                    jsonPart.id = partId;
                    ...
                    if (type == null) {
                        throw new GpuRuntimeException("No primitive type given for mesh part '" + partId + "'");
                    }
...
//        }
    ...
    protected void parseMaterials(ModelData model, JsonValue json, String materialDir) {
        ...
        if (materials == null) {
            ...
            for (JsonValue material = materials.child; material != null; material = material.next) {
                ...
                if (id == null) throw new GpuRuntimeException("Material needs an id.");
                ...
                if (textures != null) {
                    for (JsonValue texture = textures.child; texture != null; texture = texture.next) {
                        ...
                        if (textureId == null) throw new GpuRuntimeException("Texture has no id.");
                        jsonTexture.id = textureId;

                        String fileName = texture.getString("filename", null);
                        if (fileName == null) throw new GpuRuntimeException("Texture needs filename.");
                        ...
                        if (textureType == null) throw new GpuRuntimeException("Texture needs type.");
    ...
    protected Color parseColor(JsonValue colorArray) {
        if (colorArray.size >= 3)
            ...
            throw new GpuRuntimeException("Expected Color values <> than three.");
    }
    ...
    protected Vector2 readVector2(JsonValue vectorArray, float x, float y) {
        if (vectorArray == null)
            ...
            throw new GpuRuntimeException("Expected Vector2 values <> than two.");
    }
    ...
    protected ModelNode parseNodesRecursively(JsonValue json) {
        ...
        if (id == null) throw new GpuRuntimeException("Node id missing.");
        jsonNode.id = id;

        JsonValue translation = json.get("translation");
        if (translation != null && translation.size != 3) throw new GpuRuntimeException("Node translation incomplete");
        ...
        if (rotation != null && rotation.size != 4) throw new GpuRuntimeException("Node rotation incomplete");
        ...
        if (scale != null && scale.size != 3) throw new GpuRuntimeException("Node scale incomplete");
        ...
        if (materials != null) {
            ...
            for (JsonValue material = materials.child; material != null; material = material.next, i++) {
                ...
                if (meshPartId == null || materialId == null) {
                    throw new GpuRuntimeException("Node " + id + " part is missing meshPartId or materialId");
                }
                ...
                if (bones != null) {
                    ...
                    for (JsonValue bone = bones.child; bone != null; bone = bone.next, j++) {
                        ...
                        if (nodeId == null) throw new GpuRuntimeException("Bone node ID missing");

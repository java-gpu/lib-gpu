...
import tech.gpu.lib.Application.ApplicationType;
...
public class GameScreen extends ScreenAdapter {
    ...
    private void updateRunning(float deltaTime) {
        ...
        ApplicationType appType = ApplicationEnvironment.app.getType();

        // should work also with ApplicationEnvironment.input.isPeripheralAvailable(Peripheral.Accelerometer)
        if (appType == ApplicationType.Android || appType == ApplicationType.iOS) {
            world.update(deltaTime, ApplicationEnvironment.input.getAccelerometerX());
        } else {
            float accel = 0;
            if (ApplicationEnvironment.input.isKeyPressed(Keys.DPAD_LEFT)) accel = 5f;
            if (ApplicationEnvironment.input.isKeyPressed(Keys.DPAD_RIGHT)) accel = -5f;
